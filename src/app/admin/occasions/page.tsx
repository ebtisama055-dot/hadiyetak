...
export default function AdminOccasionsPage() {
  ...
  async function load() {
    setLoading(true);
    const { data } = await supabase.from('occasions').select('*').order('display_order');
    setRows(data ?? []);
    setLoading(false);
  }
  useEffect(() => { load(); }, []);
  ...
  async function updateField(id: string, field: string, value: any) {
    ...
    await supabase.from('occasions').update({ [field]: value }).eq('id', id);
  }

  async function addRow(e: React.FormEvent) {
    e.preventDefault();
    if (!newName.trim()) return;
    await supabase.from('occasions').insert({ name: newName, slug: `${slugify(newName)}-${Date.now().toString().slice(-4)}`, display_order: rows.length + 1 });
    setNewName('');
    load();
  }
  ...
  return (
    <div>
      <h1 className="font-display text-2xl font-bold mb-6">المناسبات</h1>
      <form onSubmit={addRow} className="flex gap-2 mb-6">
        <input value={newName} onChange={(e) => setNewName(e.target.value)} placeholder="اسم مناسبة جديدة" className="px-3 py-2 rounded-lg border border-blush flex-1 max-w-xs" />
        <button type="submit" className="bg-rose text-white font-bold px-4 py-2 rounded-full text-sm">+ إضافة مناسبة</button>
      </form>
